...
def make_img_overlay(img, predicted_img, white=False):
    w = img.shape[0]
    h = img.shape[1]
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    with tf.Session() as s:
        ...
        for i in range(7, 224):
            print(i)
            if not os.path.exists(test_data_filename + "test_%d" % i + ".png"):
                continue
            pimg = get_prediction_with_groundtruth(test_data_filename, i, "test_%d")
            Image.fromarray(pimg).save(prediction_test_dir + "prediction_" + str(i) + ".png")
            pimg2 = get_prediction_with_groundtruth(test_data_filename, i, "test_%d", only_prediction=True)
            Image.fromarray(pimg2).save(prediction_test_dir + "mask_" + str(i) + ".png")
